...
from lib.losses.ssd_label_loss import SSDLabelLoss
...
def main():
    ...
    parser.add_argument('-m', '--model', type=str, default='rssd', help='')
            ...
            for batch_index, (image, y_true) in enumerate(dataloader):
                ...
                if total_loss > worst_batch_loss:
                    ...
                    worst_batch = {
                        ...
                        'pred_ssd_labels': pred_labels.detach().cpu().softmax(dim=2),
                        'true_ssd_bboxes': true_bboxes.detach().cpu(),
                        'true_ssd_labels': one_hot(true_labels.detach().cpu(),
