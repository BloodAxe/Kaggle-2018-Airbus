...
class FPN(nn.Module):
    ...
    def _upsample_add(self, x, y, scale_factor=2):
        ...
        return F.interpolate(x, scale_factor=scale_factor, mode='bilinear', align_corners=True) + y
    ...
    def forward(self, x):
        ...
        p8 = self.conv8(F.relu(p7, inplace=True))
        p9 = self.conv9(F.relu(p8, inplace=True))
        # Top-down
        p5 = self.toplayer(c5)
