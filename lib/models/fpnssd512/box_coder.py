...
from lib.common.box_util import meshgrid, change_box_order, box_iou, box_nms
...
class FPNSSDBoxCoder:
    ...
    def encode(self, boxes, labels, return_anchors=False):
        ...
        if len(boxes) == 0:
            return change_box_order(self.anchor_boxes, 'xyxy2xywh'), torch.zeros((len(self.anchor_boxes)), dtype=torch.long)
    ...
    def decode(self, loc_preds, cls_preds, score_thresh=0.6, nms_thresh=0.45, offsets=None):
        ...
        Args:
          ...
          offsets: (tensor) offsets in global coordinate space for bounding boxes [#anchors,4].

        Returns:
          boxes: (tensor) bbox locations, sized [#obj,4]. Format xyxy
    ...
    def decode_multi(self, loc_preds, cls_preds, offsets, score_thresh=0.6, nms_thresh=0.45):
        '''Decode predicted loc/cls back to real box locations and class labels.
        ...
        all_box_preds = []
        all_cls_preds = []
        for loc_preds_i, cls_preds_i, offsets_i in zip(loc_preds, cls_preds, offsets):
            xy = loc_preds_i[:, :2] * anchor_boxes[:, 2:] + anchor_boxes[:, :2]
            wh = loc_preds_i[:, 2:].exp() * anchor_boxes[:, 2:]
            box_preds = torch.cat([xy - wh / 2, xy + wh / 2], 1) + offsets_i.view(1, 4)

            all_box_preds.extend(box_preds)
            all_cls_preds.extend(cls_preds_i)

        box_preds = torch.stack(all_box_preds)
        cls_preds = torch.stack(all_cls_preds)
