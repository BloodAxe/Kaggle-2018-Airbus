...
def meshgrid_numpy(x, y, row_major=True):
    '''Return meshgrid in range x & y.
    ...
    a = np.arange(0, x)
    b = np.arange(0, y)
    xx = np.tile(a, y).reshape(-1, 1)
    yy = np.tile(b.reshape(-1, 1), (1, x)).reshape(-1, 1)
    return np.concatenate([xx, yy], 1) if row_major else np.concatenate([yy, xx], 1)
